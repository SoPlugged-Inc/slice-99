    ...
    if (isOpen && !chatSession) {
      try {
        ...
        const chat = ai.chats.create({
          ...
          config: {
            systemInstruction: `You are a helpful, friendly, and professional assistant for Slice.
Slice is a service ("The Utility Layer for Creator Marketing") that helps small e-commerce businesses pool resources to afford influencer marketing.

Key details about Slice:
- Cost: $99 per slot (flat rate).
- What it is: Non-competing brands pool into one UGC-style "Haul" video.
- The Offer: Get your product on TikTok. Guaranteed.
- Process: 1) Secure slot for $99. 2) Ship product to Canadian hub. 3) Video delivered and posted by creator within 14 days.
- Guarantee: Yes, clients receive the raw video with usage rights.
- Returns: No returns, products are gifts to creators.
- Contact: support@slice99.com for support.

Answer questions concisely. If asked about booking, direct them to click the "Claim a Slot" button.
Do not make up facts not in this prompt. If unsure, ask them to email support@slice99.com.
Tone: Efficient, trustworthy, "Linear-style" professional.`,
          },
        });
        setChatSession(chat);
        setMessages([{ role: 'model', text: 'Hi there! I can help you with questions about how Slice works, shipping, or our guarantee.' }]);
      } catch (error) {
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-40 w-[90vw] max-w-sm h-[500px] bg-white rounded-xl shadow-2xl border border-neutral-lighter flex flex-col overflow-hidden animate-[slideUpFade_0.3s_ease-out]">
          ...
          <div className="bg-neutral-darkest p-4 flex items-center gap-3 border-b border-neutral-darker">
            ...
            <div>
              <h3 className="font-bold text-white text-sm">Slice Assistant</h3>
